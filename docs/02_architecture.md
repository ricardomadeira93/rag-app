...
- **UX UI Signals:** The UI interprets real-time backend metric streams to visibly render AI "Confidence" bounds (High/Med/Low dots) and semantic "Answer Types" (e.g. `[From your documents]`), providing consumer-grade visual transparency.
- **Continuous Answer Evaluation:** To govern hallucinations and quality, every AI message provides active thumbs-up/down capability, streaming evaluations instantly to an internal tracking schema (`memories.db`), equipping you with a feedback loop mechanism for future optimizations.
